...
def construct_fileset(
    ...
    if af_name == "ssl-dev":
        ...
        if local_data_cache is not None:
            raise RuntimeError(
                "`af_name='ssl-dev'` and `local_data_cache` are incompatible. Please only use one of them."
            )
        if input_from_eos:
            raise RuntimeError(
                "`af_name='ssl-dev'` and `input_from_eos` are incompatible. Please only use one of them."
            )
    ...
    for process in file_info.keys():
        ...
        for variation in file_info[process].keys():
            ...
            if use_xcache:
                file_paths = [
                    f.replace("https://xrootd-local.unl.edu:1094", "root://red-xcache1.unl.edu") for f in file_paths
                ]
            elif af_name == "ssl-dev":
                # point to local files on /data
                file_paths = [
                    f.replace("https://xrootd-local.unl.edu:1094//store/user/AGC", "/data/alheld/AGC/datasets")
                    for f in file_paths
                ]
            elif input_from_eos:
                file_paths = [
            ...
            if local_data_cache is not None:
                local_paths = [
                    f.replace("https://xrootd-local.unl.edu:1094//store/user/", f"{local_data_cache.absolute()}/")
                    for f in file_paths
                ]
