...
def get_permutations_dict(MAX_N_JETS, include_labels=False, include_eval_mat=False):
    ...
    for n in range(4, MAX_N_JETS + 1):
        ...
        permutations = ak.concatenate(
            [test[unzipped[i][different]][..., np.newaxis] for i in range(len(unzipped))], axis=1
        ).to_list()
...
def get_features(jets, electrons, muons, max_n_jets=6):
    ...
    features[:, 0] = ak.flatten(
        np.sqrt((leptons.eta - jets[perms[..., 3]].eta) ** 2 + (leptons.phi - jets[perms[..., 3]].phi) ** 2)
    ).to_numpy()
    ...
    el_p4 = ak.zip(
        {"pt": electrons.pt, "eta": electrons.eta, "phi": electrons.phi, "mass": electrons.mass}, with_name="Momentum4D"
    )
...
def get_inference_results_local(features, even, model_even, model_odd):
    ...
    if len(features[np.invert(even)]) > 0:
        results[np.invert(even)] = model_even.predict_proba(features[np.invert(even), :])[:, 1]
    return results


def get_inference_results_triton(features, even, triton_client, MODEL_NAME, MODEL_VERS_EVEN, MODEL_VERS_ODD):
...
def training_filter(jets, electrons, muons, genparts, even):
    ...
    grandchild_pdgid = (
        nearest_parent.distinctChildren.distinctChildren.pdgId
    )  # pdgId of particle's parent's grandchildren
    ...
    has_lepton_cousin = (
        ak.sum(((grandchildren_flat % 2 == 0) & (grandchildren_flat > 10) & (grandchildren_flat < 19)), axis=-1) > 0
    )
    # if particle has a cousin that is a neutrino
    has_neutrino_cousin = (
        ak.sum(((grandchildren_flat % 2 == 1) & (grandchildren_flat > 10) & (grandchildren_flat < 19)), axis=-1) > 0
    )
...
def get_training_set(jets, electrons, muons, labels):
    ...
    which_anti_combination = list(
        zip(
            range(
                labels.shape[0],
            ),
            which_anti_combination,
        )
    )
...
def write_triton_config(
    model_name, n_features, backend_name="fil", model_type="xgboost", max_batch_size=50000000, predict_proba="true"
):
    n_out = 1
