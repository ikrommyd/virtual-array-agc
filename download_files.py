...
def handle_target_directory(target_dir, force_recreate):
    ...
    if os.path.exists(target_dir):
        if is_directory_empty(target_dir):
            print(
                f"Directory '{target_dir}' exists and is empty. Proceeding with download."
            )
        else:
            if force_recreate:
                print(
                    f"Directory '{target_dir}' exists and is not empty. Force recreating by deleting its contents."
                )
                try:
                ...
                print(
                    f"Directory '{target_dir}' exists and is not empty. Use --force-recreate to overwrite."
                )
                sys.exit(1)
    else:
        try:
...
def download_file(file_url, destination_folder, key):
    ...
    try:
        ...
        # Define the wget comamnd
        command = ['wget', '-O', destination_path, file_url]

        # Execute the wget command to copy the file
...
def download_files_in_parallel(file_dict, target_dir, num_files, num_threads):
    ...
    for key, files in file_dict.items():
        ...
        if actual_num_files > len(files):
            print(
                f"Requested {actual_num_files} files for key '{key}', but only {len(files)} available. Proceeding to download all available files."
            )
...
def main():
    ...
    Workflow:
        ...
        5. Download the specified number of files using wget, either sequentially or in parallel.
    """
    # Initialize the argument parser with a description of the script
    parser = argparse.ArgumentParser(
        description="Download files from a JSON file using wget"
    )
    ...
    parser.add_argument(
        "--key",
        required=True,
        help='Key in the JSON file to process or "all" to process all keys',
    )
    parser.add_argument(
        "--num-files", type=int, default=None, help="Number of files to process per key"
    )
    parser.add_argument(
        "--target-dir", required=True, help="Target directory to copy files into"
    )
    parser.add_argument(
        "--force-recreate",
        "-f",
        action="store_true",
        help="Force recreate the target directory by deleting its contents if it exists",
    )
    parser.add_argument(
        "--num-threads",
        type=int,
        default=8,
        help="Number of parallel threads to use for downloading",
    )
    ...
    if args.key == "all":
        ...
        for key in keys:
            ...
            if isinstance(file_list, dict):
                file_list = file_list.get("files", None)
            ...
            download_files_in_parallel(
                {key: file_list}, sub_target_dir, args.num_files, args.num_threads
            )
    else:
        ...
        # Download the files in parallel
        download_files_in_parallel(
            {args.key: file_list}, args.target_dir, args.num_files, args.num_threads
        )
